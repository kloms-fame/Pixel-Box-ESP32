const BLEManager = {
    ...
    async disconnect() { if (this.device && this.device.gatt.connected) await this.device.gatt.disconnect(); },
    async sendCmd(cmdArray) { if (this.rxChar) await this.rxChar.writeValue(new Uint8Array(cmdArray)); }
};